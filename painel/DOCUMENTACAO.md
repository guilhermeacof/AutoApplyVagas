...
| `extensao/` | Extensão de navegador que preenche a tela de candidatura com as respostas salvas. Funciona em **qualquer site**, acionada pelo **ícone** (`activeTab`): as vagas vêm de 60+ domínios e o anúncio redireciona para o ATS da empresa, então lista fixa de sites não cobre — e rodar em todos os sites daria à extensão acesso a banco/e-mail. **Nunca envia** — preenche e para. Instalação em `extensao/LEIA-ME.txt`. |
...
| `GET /api/answers/for?url=…&titulo=…` | Respostas para a vaga que a **extensão** está vendo no portal. Três tentativas: URL idêntica → **id da vaga** (`idVaga()`: `/jobs/view/…-<id>`, `currentJobId=<id>`, `jobId` no base64 da Gupy) → **título da página** (`cobertura()` exige empresa **e** cargo ≥ 0.5). O terceiro existe porque anúncio e formulário costumam estar em **domínios diferentes** (o radar guarda a vaga do Serasa no `br.linkedin.com`, mas candidatar-se leva ao `jobs.smartrecruiters.com`, com outro id) — aí só o texto liga as duas. Sem nada: `{achou:false, campos:<padrão>}` e a extensão oferece a escolha manual. |
| `GET /api/answers/list` | Vagas que têm respostas salvas (`{vagas:[{url,title,company,campos,atualizadoEm}]}`, mais recentes primeiro). É o seletor que a extensão mostra quando não reconhece a página. |

> **O padrão SOMA à vaga, não é alternativa a ela.** `/api/answers/for` devolve `vaga.campos` + os campos do padrão que a vaga não tem (a vaga sempre ganha). Uma vaga congela o padrão do dia em que foi preparada: sem a soma, um campo criado depois (nome, e-mail, telefone) nunca chegaria às vagas antigas — no formulário do SmartRecruiters só o LinkedIn era preenchido. Os rótulos do padrão são **bilíngues** (`Nome (First name)`) porque o casamento é por semelhança de palavras e formulário de multinacional é em inglês: sem a palavra em inglês, `City*` não casa com `Cidade / Estado`.
