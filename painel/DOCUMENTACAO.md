...
| `GET /api/state` | Estado do workspace: cargo, perfil (CV enviado? perfil pronto?), resumo (nº de vagas, candidaturas, prazos), lista de vagas ranqueadas (com `gaps` e as flags `aplicada`/`preparada`) e `aplicadasUrls`. **`aplicada` ≠ `preparada`**: estar no tracker não quer dizer enviada — linha `status: prepared` é "documentos prontos, falta a pessoa enviar no portal"; qualquer outro status conta como enviada (ver `ehPreparada()`/`markApplied()`). |
